...
document
  ...
  .addEventListener('input', () => validateInput('firstNameInput', 'firstNameError'));
document
  .getElementById('lastNameInput')
  .addEventListener('input', () => validateInput('lastNameInput', 'lastNameError'));
document
  .getElementById('emailInput')
  .addEventListener('input', () => validateInput('emailInput', 'emailError'));
document
  .getElementById('phoneInput')
  .addEventListener('input', () => validateInput('phoneInput', 'phoneError'));
document
  .getElementById('subject')
  .addEventListener('input', () => validateInput('subject', 'subjectError'));
document
  .getElementById('message')
  .addEventListener('input', () => validateInput('message', 'messageError'));

// Event listener for the form submission
document.getElementById('wf-form-Contact-Form').addEventListener('submit', function (event) {
  let hasError = false;

  // Check each input for bad words before submitting
  ['firstNameInput', 'lastNameInput', 'emailInput', 'phoneInput', 'subject', 'message'].forEach(
    (inputId) => {
      const errorId = inputId.replace('Input', '') + 'Error'; // Adjust error IDs as necessary
      const inputElement = document.getElementById(inputId);
      if (inputElement && containsBadWords(inputElement.value)) {
        toggleErrorMessage(errorId, 'Inappropriate language detected.');
        hasError = true;
      } else {
  ...
  // If any errors, prevent the form from submitting and stop event propagation
  if (hasError) {
